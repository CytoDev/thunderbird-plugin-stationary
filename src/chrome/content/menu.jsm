...
Stationery.definePreference("AttachMenu_3paneWrite", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_3paneReply", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_3paneReplyAll", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_3paneForward", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_3panehdrReply", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_3panehdrForward", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_MsgViewWrite", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_MsgViewReply", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_MsgViewReplyAll", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_MsgViewForward", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_MsgViewhdrReply", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_MsgViewhdrForward", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_StationeryOptions", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_ComposerChangeStationery", {
    type   : "bool",
    default: true
});

Stationery.definePreference("AttachMenu_ComposerStationeryToolbutton", {
    type   : "bool",
    default: true
});
...
const prefObserver = Stationery.registerPreferenceObserver("", {
    ...
    observe: function(aSubject, aTopic, aData) {
        ...
        if (aData.match(/AttachMenu_/)) {
            updateAllStationeryMenus();
        }
    }
}, true);
    ...
    windowLoaded: function(win) {
        ...
        if (Stationery.isMessengerWindow(win) || Stationery.isMessageWindow(win) || Stationery.isComposerWindow(win)) {
            ...
            win.document.documentElement.appendChild(Stationery.makeElement(win.document, "tooltip", {
                ...
                events: [
                    {
                        name : "popupshowing",
                        value: Stationery.templates.onTemplateMenuitemTooltipShowing
                    }
                ]
            }));
        }
        ...
        if (Stationery.isMessengerWindow(win) || Stationery.isMessageWindow(win)) {
            ...
            win.setInterval(function() {
                sanitizeToolbarBecauseOfCompactHeaderExtension(win);
            }, 1000);
...
Stationery.setupElement = function(element, v) {
    ...
    if ("label" in v) {
        element.setAttribute("label", v.label);
    }

    if ("tooltip" in v) {
        element.tooltipText = v.tooltip;
    }

    if ("class" in v) {
        Stationery.addClass(element, v.class);
    }

    if ("attr" in v) {
        for (let a of fixIterator(v.attr)) {
            if ("remove" in a) {
...
function menupopupHaveOneNonStationeryItem(menupopup, id) {
    ...
    for (let i = 0; i < nodes.length; ++i) {
        ...
        if (node.hasAttribute("stationery-menuitem")) {
            continue;
        }
...
function updateStationeryMenu(win, id) {
    try {
        ...
        if (id === "button-newmsg"
            ...
            if (showStationeryMenu || haveOneNonStationeryItem) {
                ...
                if (!givenObject.hasAttribute("oncommand")) {
                    if (id === "button-reply") {
                        Stationery.setupElement(givenObject, {
                            attr: [{
                                name : "oncommand",
                                value: "MsgReplyMessage(event)"
                            }]
                        });
                    }
                    ...
                    if (id === "button-replyall") {
                        Stationery.setupElement(givenObject, {
                            attr: [{
                                name : "oncommand",
                                value: "MsgReplyToAllMessage(event)"
                            }]
                        });
                    }
        ...
        if (id === "msgComposeContext") {
            ...
            if (subMenu) {
                subMenu.setAttribute("collapsed", showStationeryMenu ? "false" : "true");
            }

            if (topSeparator) {
                topSeparator.setAttribute("collapsed", showStationeryMenu ? "false" : "true");
            }
        }
        ...
        if (id === "composeToolbar2") {
            ...
            if (showStationeryMenu) {
                ...
                if ("templateCanBeChanged" in win.Stationery_) {
                    ...
                    if (btn) {
                        Stationery.enableOrDisableElement(btn, win.gMsgCompose.composeHTML && win.Stationery_.templateCanBeChanged);
                    }
                } else {
                    delayedupdateStationeryMenuTimer.startTimeout(function() {
                        updateStationeryMenu(win, id);
                    }, 500);
                }
            } else {
...
function onStationeryMenuParentPopup(event) {
    try {
        ...
        if (id === "msgComposeContext") {
            ...
            if (menu) {
                menu.setAttribute("collapsed", showStationeryMenu ? "false" : "true");
            }
            ...
            if (menuSep) {
                menuSep.setAttribute("collapsed", showStationeryMenu ? "false" : "true");
            }
        ...
        if (id === "hdrReplyButton"
            ...
            if (menuSep) {
                menuSep.setAttribute("collapsed", showTopSeparator ? "false" : "true");
            }
...
function onStationeryMenuPopup(event) {
    try {
        ...
        if (id === "button-forward" || id === "hdrForwardButton") {
            flag = "notForForward";
        }
        ...
        if (menuSep) {
            menuSep.setAttribute("collapsed", String(!showTopSeparator));
        }
        ...
        for (let template of Stationery.templates.getTemplatesIterator(identityKey)) {
            //filter by flags
            if (Stationery.templates.haveFlag(template, flag)) {
                continue;
            }
            ...
            menupopup.appendChild(Stationery.makeElement(doc, "menuitem", {
                ...
                attr  : [
                    {
                        name : "stationery-menuitem",
                        value: "true"
                    },
                    {
                        name : "stationery-template",
                        value: template.uid
                    },
                    {
                        name : "stationery-related-id",
                        value: id
                    },
                    {
                        name : "tooltip",
                        value: "stationery-menu-tooltip"
                    }
                ],
                events: [
                    {
                        name : "command",
                        value: Stationery.templates.onTemplateMenuitemCommand
                    }
                ]
            }));
        }
