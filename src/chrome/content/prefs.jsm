...
Stationery.getPref = function(name) {
    return getPref(name, valuesBranch);
};

Stationery.setPref = function(name, value) {
    setPref(name, value, valuesBranch);
};

Stationery.definePreference = function(prefName, v) {
    dp(prefName, v);
};

Stationery.registerPreferenceObserver = function(prefName, observer, aHoldWeak) {
    valuesBranch.addObserver(prefName, observer, aHoldWeak);

    return observer;
};

Stationery.unRegisterPreferenceObserver = function(prefName, observer) {
    valuesBranch.removeObserver(prefName, observer);
...
function isValidPrefType(name, preference, branch) {
    ...
    if (existingPrefType === branch.PREF_INVALID) {
        return true;
    }

    if (preference.type === PT_BOOL) {
        return existingPrefType === branch.PREF_BOOL;
    }

    if (preference.type === PT_INT) {
        return existingPrefType === branch.PREF_INT;
    }
...
function getPref(name, branch) {
    try {
        ...
        if (!isValidPrefType(name, preference, branch)) {
        ...
        if (branch.getPrefType(name) !== branch.PREF_INVALID) {
            switch (preference.type) {
                ...
                case PT_JSON:
                    return JSON.parse(branch.getCharPref(name));
            }
        }
...
const encode_replacement = function(c) {
...
function dp(prefName, v) {
    ...
    if ("type" in v) {
        switch (v["type"].toLowerCase()) {
            case "string":
                preference.type = PT_STRING;
                break;
            case "int":
                preference.type = PT_INT;
                break;
            case "bool":
                preference.type = PT_BOOL;
                break;
            case "unicode":
                preference.type = PT_UNICODE;
                break;
            case "json":
                preference.type = PT_JSON;
                break;
        }
    }
        ...
        beforeInitWindow: function() {
            ...
            ["AutomaticManagement", "ChangeConfirmation", " ApplyStationery_New", "ApplyStationery_MailToUrl", "",
             ...
             "AttachMenu_MsgViewReplyAll", "AttachMenu_MsgViewForward", "AttachMenu_MsgViewhdrReply"
                ...
                try {
                    if (oldBranch.getPrefType(i) === valuesBranch.PREF_BOOL) {
                        Stationery.setPref(i, oldBranch.getBoolPref(i));
                    }
                } catch (e) {
                    ...
                    if (oldBranch.getPrefType("Template" + i) !== valuesBranch.PREF_INVALID) {
                        ...
                        if (tmp !== "") {
                            if ((-1 !== tmp.indexOf(Stationery.getFilePathSeparator())) || (tmp.substr(0, 11) === "profile:///")) {
                                templates["*"].push({
                                    type: "file",
                                    name: makeAbbrevTemplateName(tmp),
                                    url : tmp
                                });
                            } else if (!blankFound) {
                                //add only first 'blank', rest are trash
                                blankFound = true;
            ...
            try {
                Stationery.setPref("Templates", templates);
            } catch (e) {
...
Stationery.definePreference("AutomaticManagement", {
    type   : "bool",
    default: true
});

Stationery.definePreference("ChangeConfirmation", {
    type   : "bool",
    default: false
});

Stationery.definePreference("ChangeTemplateWithIdentity", {
    type   : "bool",
    default: true
});

Stationery.definePreference("ChangeTemplateWithIdentityConfirmation", {
    type   : "bool",
    default: true
});

Stationery.definePreference("TemplatesCount", {
    type   : "int",
    default: 40
});

Stationery.definePreference("Templates", {
    type   : "json",
    default: {"*": []}
});

Stationery.definePreference("DefaultSearchPath", {
    type   : "unicode",
    default: ""
});

Stationery.definePreference("DefaultTemplateEncoding", {
    type   : "string",
    default: "UTF-8"
});

Stationery.definePreference("AddresingWidgetLines", {
    type   : "int",
    default: 0
});

Stationery.definePreference("SourceEditEnabled", {
    type   : "bool",
    default: false
});
