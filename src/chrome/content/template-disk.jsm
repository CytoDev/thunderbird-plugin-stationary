...
function readHTMLTemplateFile(template) {
    try {
        ...
        try {
            ...
            if (bom.charCodeAt(0) === 239 && bom.charCodeAt(1) === 187 && bom.charCodeAt(2) === 191) {
                bomCharset = "UTF-8";
            } //UTF-8 BOM

            if (bom.charCodeAt(0) === 255 && bom.charCodeAt(1) === 254) {
                bomCharset = "UTF-16LE";
            }  //UTF-16 LE BOM

            if (bom.charCodeAt(0) === 254 && bom.charCodeAt(1) === 255) {
                bomCharset = "UTF-16BE";
            }  //UTF-16 BE BOM
