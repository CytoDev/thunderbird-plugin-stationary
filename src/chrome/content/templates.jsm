...
Stationery.templates = {
    ...
    getTemplateByUid: function(uid) {
        return copyTemplate(getTemplateByUid(uid), "deep");
    },

    getTemplateIdentityByUid: function(uid) {
        return getTemplateIdentityByUid(uid);
    },
    ...
    enterEditmode: function(templateOrUid) {
        if (!st.isEditable(templateOrUid)) {
            return;
        }
    ...
    haveFlag: function(template, flag) {
        if (typeof template.flags != "object") {
            template.flags = {};
        }
    ...
    onTemplatePropertyEdited: function(templateUid, propertyName, newValue) {
        ...
        if (propertyName === "type" || propertyName === "url") {
            return null;
        }
    ...
    makeIdentityUseGlobalList: function(identityKey) {
        ...
        Stationery.notifyOptions({
            type : "identities.removed",
            value: identityKey
        });
    },
    ...
    isIdentityUsed: function(identityKey) {
        return identityKey in templates;
    },

    //return iterator to templates list for identity. mainly used to build menu
    getTemplatesIterator: function(identityKeyOrWindow) {
        return fixIterator(getListForIdentity(identityKeyOrWindow));
    },
    ...
    getIdentityKey: function(identityKeyOrWindow) {
        ...
        if (Stationery.isWindow(identityKeyOrWindow)) {
            ...
            if (Stationery.isMessengerWindow(win)) {
                ...
                try {
                    identityKey = win.accountManager.getFirstIdentityForServer(win.GetDefaultAccountRootFolder().server).key;
                } catch (e) {
                ...
                if (win.gFolderDisplay.displayedFolder) {
                    try {
                        identityKey = win.accountManager.getFirstIdentityForServer(win.gFolderDisplay.displayedFolder.server).key;
                    } catch (e) {
                ...
                if (win.gFolderDisplay.selectedMessage) {
                    try {
                        identityKey = win.accountManager.getFirstIdentityForServer(win.gFolderDisplay.selectedMessage.folder.server).key;
                    } catch (e) {
    ...
    getHandlerMenuitemIterator: function(doc) {
        ...
        for (let i = 0; i < handlerTypes.length; ++i) {
            ...
            if (menuitem) {
                result.push(Stationery.setupElement(menuitem, {
                    attr  : [{
                        name : "stationery-handler-type",
                        value: handlerTypes[i]
                    }],
                    events: [{
                        name : "command",
                        value: Stationery.templates.onHandlerMenuitemCommand
                    }]
                }));
            }
        }
    ...
    onTemplateMenuitemCommand: function(event) {
        try {
            if (isComposerContext(event) && !confirmChange(event)) {
                return;
            }
            ...
            if (isComposerContext(event)) {
                event.view.Stationery_.ApplyTemplate(true);
            }
        } catch (e) {
    ...
    onTemplateMenuitemTooltipShowing: function(event) {
        try {
            ...
            const div = Stationery.makeElement(tooltipElement, "html:div", {
                attr: [{
                    name : "flex",
                    value: "1"
                }]
    ...
    load: function(identityKeyOrWindow, template) {
        ...
        if ("loadingError" in template) {
            const button = Services.prompt.confirmEx(
                null,
                "Stationery",
                Stationery._f("template.loading.error", [template.loadingError]),
            ...
            if (button === 0) {
                Stationery.templates.remove(template);
            }
...
const blankTemplate = {
    type: "blank",
    name: Stationery._("template.blank.name"),
    url : "blank"
};
...
st.registerHandler({
    ...
    getTip: function(template) {
        return Stationery._("template.blank.tip");
    },
...
function copyTemplate(template, reason) {
    ...
    if (reason === "restore" || reason === "clear") {
        ...
        if (reason === "clear" && "uid" in template) {
...
function isComposerContext(event) {
    return event.target.hasAttribute("stationery-related-id")
        && (event.target.getAttribute("stationery-related-id") === "msgComposeContext" || event.target.getAttribute("stationery-related-id") === "composeToolbar2");
}

function confirmChange(event) {
    if (!event.view.gMsgCompose.bodyModified) {
        return true;
    }
...
function fixSingleIdentyList(list) {
    try {
        ...
        if (!blankFound) {
            --countLimit;
        } //make space for blank template

        list.splice(countLimit, 999); //remove items over limit

        if (!blankFound) {
            list.push(copyTemplate(blankTemplate, "deep"));
        }
    } catch (e) {
        Stationery.handleException(e);
    }
...
Stationery.templates.__loadFromPrefs_for_prefs_jsm = function() {
    try {
        loadFromPrefs();
    } catch (e) {
