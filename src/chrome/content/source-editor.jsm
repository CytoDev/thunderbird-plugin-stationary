...
Stationery.definePreference("SourceEditOptions", {
    type   : "json",
    default: {
        wordWrap: true,
        base    : {
    ...
    initWindow: function(win) {
        if (Stationery.isSourceEditorWindow(win)) {
            ...
            editor.commands.addCommand({
                ...
                exec    : function(editor) {
                    editor.commands.exec(!editor.getLastSearchOptions().needle ? "find" : "findnext", editor);
                },
                readOnly: true
            });
        }
        ...
        try {
            ...
            if (typeof prefs === "object") {
                ...
                if (typeof prefs.base === "object") {
                    if ("f" in prefs.base) {
                        editorOptions.fontFamily = prefs.base.f;
                    }

                    if ("fs" in prefs.base) {
                        editorOptions.fontSize = prefs.base.fs + "pt";
                    }
                }
    ...
    editor.getSession().on("changeFold", function(param) {
        if (param.action === "remove" && param.data.range.dataUri) {
            Stationery.makeTimer().startTimeout(function() {
                ...
                if (range.toScreenRange(editor.getSession()).isEmpty()) {
                    return;
                }
