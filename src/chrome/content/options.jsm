...
Stationery.showOptions = function(eventOrWin) {
    ...
    if (Stationery.isWindow(eventOrWin)) {
        win = eventOrWin;
    }

    if (eventOrWin && eventOrWin.view) {
        win = eventOrWin.view;
    }
...
Stationery.notifyOptions = function(v) {
    if (!v || !v.type || !Stationery.optionsTab || !Stationery.optionsTabNotification) {
        return;
    }
...
function makeTabType(win) {
    return {
        ...
        perTabPanel: "vbox",
        modes      : {
            "extensions.stationery.options": {
                type   : "extensions.stationery.options",
                maxTabs: 1
            }
        },
        ...
        openTab: function(aTab, aArgs) {
            ...
            try {
                ...
                //ignore exceptions, in case if in new TB version they change toolbox
            }
            ...
            Stationery.setupElement(aTab.browser, {
                attr: [
                    {
                        name : "oncommand",
                        value: "specialTabs.defaultClickHandler(event)"
                    },
                    {
                        name : "id",
                        value: "chromeTabBrowser-stationery"
                    }
                ]
            });
