...
Stationery.RegisterXPCOM = function(interfaceName, componentString, interfaceType) {
    xpcomComponents[interfaceName] = {
        comStr: componentString,
        iface : interfaceType
    };
};
...
Stationery.XPCOM = function(interfaceName) {
    ...
    if (!o) {
        throw Components.Exception("Stationery.XPCOM() >> unregistred component: \"" + interfaceName + "\"!");
    }
...
Stationery._ = function(string) {
    try {
        return l10n.GetStringFromName(string);
    } catch (e) {
        Stationery.handleException(e, string);

        return string;
    }
};

Stationery._f = function(string, args) {
    try {
...
Stationery.hasClass = function(e, c) {
    return e.className.match(new RegExp("(\\s|^)" + c + "(\\s|$)"));
};

Stationery.addClass = function(e, c) {
    if (!Stationery.hasClass(e, c)) e.className += " " + c;
};

Stationery.removeClass = function(e, c) {
    if (Stationery.hasClass(e, c)) e.className = e.className.replace(new RegExp("(\\s|^)" + c + "(\\s|$)"), " ");
};
        ...
        notify: function(aTimer) {
            if (typeof this.code == "function") {
                try {
                    ...
                    if (this.nsITimer.type === this.nsITimer.TYPE_ONE_SHOT) {
                        this.code = null;
                    }
...
Stationery.getWindowType = function(win) {
    if (!Stationery.isWindow(win) || !win.document.documentElement.hasAttribute("windowtype")) {
        return false;
    }
...
Stationery.isMessengerWindow = function(win) {
    return Stationery.getWindowType(win) === "mail:3pane";
};

Stationery.isComposerWindow = function(win) {
    return Stationery.getWindowType(win) === "msgcompose";
};

Stationery.isMessageWindow = function(win) {
    return Stationery.getWindowType(win) === "mail:messageWindow";
};

Stationery.isOptionsWindow = function(win) {
    return Stationery.getWindowType(win) === "stationery:optionsPage";
};

Stationery.isSourceEditorWindow = function(win) {
    return Stationery.getWindowType(win) === "editor:source";
};
...
Stationery.guessContentType = function(content /*binary string*/, defaultType) {
    ...
    if (c0 === "\xFF" && c1 === "\xD8" && c2 === "\xFF") {
        return "image/jpeg";
    }

    if (c0 === "\x89" && c1 === "P" && c2 === "N" && c3 === "G" && c4 === "0D" && c4 === "0A") {
        return "image/png";
    }
