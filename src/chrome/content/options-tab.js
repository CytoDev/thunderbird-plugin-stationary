...
Stationery.definePreference("lastCategory", {
    type   : "int",
    default: 0
});
...
function updateIdentitiesSelector(selectedId) {
    ...
    idSelectorMenupopup.appendChild(Stationery.makeElement(document, "menuitem", {
        ...
        events: [{
            name : "command",
            value: Stationery_.onIdSelectorCommand
        }]
    }));
            ...
            for (let template of Stationery.templates.getTemplatesIterator(identityKey)) {
                templatesList.appendChild(Stationery.makeElement(document, "richlistitem", {
                    ...
                    attr  : [
                        ...
                        {
                            name : "stationery-template-type",
                            value: template.type
                        },
                        {
                            name : "stationery-template-url",
                            value: Stationery.templates.getDisplayUrl(template)
                        },
                        {
                        ...
                        {
                            name    : "stationery-for-new-mail",
                            value   : !Stationery.templates.haveFlag(template, "notForNewMail"),
                            checkbox: true
                        },
                        {
                            name    : "stationery-for-reply",
                            value   : !Stationery.templates.haveFlag(template, "notForReply"),
                            checkbox: true
                        },
                        {
                            name    : "stationery-for-forward",
                            value   : !Stationery.templates.haveFlag(template, "notForForward"),
                            checkbox: true
                        }
                    ...
                    events: [
                        {
                            name : "dragstart",
                            value: Stationery_.onTemplateItemStartDrag
                        },
                        {
                            name : "dragover",
                            value: Stationery_.onTemplateItemDragOver
                        },
                        {
                            name : "drop",
                            value: Stationery_.onTemplateItemDragDrop
                        },
                        {
                            name : "dragend",
                            value: Stationery_.onTemplateItemEndDrag
                        },
                        {
                            name : "dblclick",
                            value: Stationery_.onTemplateDblClicked
                        }
                    ]
                }));
            }
...
Stationery_.onTemplateItemDragOver = function(event) {
    ...
    if (diff < 0) {
        richListItem.setAttribute("drop-feedback", "top");
    }

    if (diff > 0) {
        richListItem.setAttribute("drop-feedback", "bottom");
    }
};
...
Stationery.optionsTabNotification = function(v) {
    ...
    switch (v.type) {
        case "identities.added":
            updateIdentitiesSelector(v.value);
            break;
        case "identities.removed":
            updateIdentitiesSelector("*");
            break;
        case "templates.changed":
            reFillTemplatesList();
            break;
        case "templates.enter.edit":
            reFillTemplatesList();
            break;
        case "templates.leave.edit":
            reFillTemplatesList();
            break;
    }
};
...
function onTemplatesListSelectionChanged() {
    ...
    Stationery.enableOrDisableElement(
        document.getElementById("stationery-button-up-template"),
        selectedIndex > -1 && templatesList.selectedIndex > 0
    );

    Stationery.enableOrDisableElement(
        document.getElementById("stationery-button-down-template"),
        selectedIndex > -1 && templatesList.selectedIndex < templatesList.getRowCount() - 1
    );

    Stationery.enableOrDisableElement(
        document.getElementById("stationery-button-edit-template"),
        selectedIndex > -1 && Stationery.templates.isEditable(selectedTemplateUid) && Stationery.templates.editedUid() == null
    );

    Stationery.enableOrDisableElement(
        document.getElementById("stationery-button-edit-template-finish"),
        Stationery.templates.editedUid() != null
    );

    Stationery.enableOrDisableElement(
        document.getElementById("stationery-button-remove-template"),
